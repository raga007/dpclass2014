...
public class TestGson {
  public static void main(String [] args) {
    ...
    BT root = new BT("root", null, null );
    BT rootL = new BT( "L", null, null );
    BT rootR = new BT( "R", null, null );
    BT common = new BT("common", new BT("cl", null, null), new BT("cr", null, null));
    root.left = rootL;
    root.right = rootR;
    rootL.right = common;
    rootR.left = common;
    Set<BT> nodes = new HashSet<BT>();
    BT.numNodes( root, nodes );
    System.out.println("old tree has " + nodes.size() );

    String treeString = gson.toJson(root);
    System.out.println("tree as json:" + treeString);
    BT desTree = gson.fromJson(treeString, BT.class);
    System.out.println("deser tree :" + desTree.toString()); 

    Set<BT> newnodes = new HashSet<BT>();
    BT.numNodes( desTree, newnodes );
    System.out.println("new tree has " + newnodes.size() );
...
class BT {
  ...
  String key;
  BT() { }
  BT(String key, BT left, BT right ) {
    this.key = key;
    this.left = left;
    this.right = right;
  }
  ...
  public boolean equals(Object o) {
    BT bo = (BT) o;
    if ( !bo.key.equals( key ) ) {
      return false;
    }
    if ( bo.left != null && left == null || left != null && bo.left == null ) {
      return false;
    }
    if (!( bo.left == null && left == null )) {
      if ( !bo.left.equals( left ) ) {
        return false;
      }
    }
    if ( bo.right != null && right == null || right != null && bo.right == null ) {
      return false;
    }
    if (!( bo.right == null && right == null )) {
      if ( !bo.right.equals( right ) ) {
        return false;
      }
    }
    return true;
  ...
  static void numNodes( BT t, Set<BT> nodes ) {
    if ( t == null ) 
      return;

    nodes.add( t );
    numNodes( t.left, nodes );
    numNodes( t.right, nodes );
  }
}
